...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Ok(t) => t,
        Err(e) => match e {
            TicketNewError::TitleError { d } => panic!("{}", d),
            TicketNewError::DescriptionError { d } => Ticket { title, description: "Description not provided".into(), status },
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { d: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { d: "Title cannot be longer than 50 bytes".to_string() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { d: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { d: "Description cannot be longer than 500 bytes".to_string() });
        }
